...
if [[ ! "${WPE_P006_TABLE_PREFIX}" =~ ^wpep006b6b_$ ]]; then
  echo "invalid B6b table prefix" >&2
  exit 1
fi

wp_dir="${WPE_TEST_WORDPRESS_DIR}"
archive="/tmp/p006-b6b-wordpress-${WPE_P006_EXPECTED_WP}.tar.gz"

rm -rf "${wp_dir}" "${archive}"
curl --proto '=https' --tlsv1.2 --fail --silent --show-error --location --retry 3   "https://wordpress.org/wordpress-${WPE_P006_EXPECTED_WP}.tar.gz"   --output "${archive}"
...
define('DB_COLLATE', '');
define('AUTH_KEY', 'p006-b6b-auth-key');
define('SECURE_AUTH_KEY', 'p006-b6b-secure-auth-key');
define('LOGGED_IN_KEY', 'p006-b6b-logged-in-key');
define('NONCE_KEY', 'p006-b6b-nonce-key');
define('AUTH_SALT', 'p006-b6b-auth-salt');
define('SECURE_AUTH_SALT', 'p006-b6b-secure-auth-salt');
define('LOGGED_IN_SALT', 'p006-b6b-logged-in-salt');
define('NONCE_SALT', 'p006-b6b-nonce-salt');
...
if (!is_string($tablePrefix) || $tablePrefix !== 'wpep006b6b_') {
    die('Invalid B6b table prefix');
}
$table_prefix = $tablePrefix;
require_once ABSPATH . 'wp-settings.php';
PHP

cat > "${wp_dir}/wp-content/mu-plugins/p006-b6b-network-block.php" <<'PHP'
<?php
if (!defined('P006_B6B_NETWORK_BLOCK_ACTIVE')) {
    define('P006_B6B_NETWORK_BLOCK_ACTIVE', true);
}
add_filter('pre_http_request', static function ($preempt, $parsedArgs, $url) {
    ...
    return new WP_Error('p006_b6b_network_blocked', 'Outbound WordPress HTTP is denied in B6b prerequisite evidence.');
}, PHP_INT_MIN, 3);
PHP

: > "${WPE_P006_NETWORK_LOG}"
test -f "${wp_dir}/wp-config.php"
test -f "${wp_dir}/wp-content/mu-plugins/p006-b6b-network-block.php"
