...
archive="/tmp/p006-wave1k-wordpress-${WPE_P006_EXPECTED_WP}.tar.gz"
...
extract_node "${WPE_P006_F1_ZIP_PATH}" f1a wpessential wpessential.php
extract_node "${WPE_P006_F1_ZIP_PATH}" f1b wpessential wpessential.php
extract_node "${WPE_P006_P0_ZIP_PATH}" p0 wpessential-pro wpessential-pro.php
extract_node "${WPE_P006_P1_ZIP_PATH}" p1 wpessential-pro wpessential-pro.php

ln -s "../p006-wave1k-packages/f0/wpessential" "${plugins_dir}/wpessential"
ln -s "../p006-wave1k-packages/p0/wpessential-pro" "${plugins_dir}/wpessential-pro"
...
define('DB_COLLATE', '');
define('AUTH_KEY', 'p006-wave1k-auth-key');
define('SECURE_AUTH_KEY', 'p006-wave1k-secure-auth-key');
define('LOGGED_IN_KEY', 'p006-wave1k-logged-in-key');
define('NONCE_KEY', 'p006-wave1k-nonce-key');
define('AUTH_SALT', 'p006-wave1k-auth-salt');
define('SECURE_AUTH_SALT', 'p006-wave1k-secure-auth-salt');
define('LOGGED_IN_SALT', 'p006-wave1k-logged-in-salt');
define('NONCE_SALT', 'p006-wave1k-nonce-salt');
define('WP_DEBUG', false);
define('DISABLE_WP_CRON', true);
define('AUTOMATIC_UPDATER_DISABLED', true);
define('WP_AUTO_UPDATE_CORE', false);
define('WP_ENVIRONMENT_TYPE', 'development');
$table_prefix = 'wpep006k_';
require_once ABSPATH . 'wp-settings.php';
PHP

cat > "${wp_dir}/wp-content/mu-plugins/p006-wave1k-core-install-network-deny.php" <<'PHP'
<?php
add_filter('pre_http_request', static function ($preempt, $parsedArgs, $url) {
    if (function_exists('wp_installing') && wp_installing()) {
        return new WP_Error(
            'p006_wave1k_core_install_network_blocked',
            'Core installer HTTP is denied inside the disposable P-006 Wave 1K fixture.'
        );
    }
    return $preempt;
}, PHP_INT_MIN, 3);
...
cat > /tmp/p006-wave1k-core-install.php <<'PHP'
<?php
...
php /tmp/p006-wave1k-core-install.php

rm -f   /tmp/p006-wave1k-core-install.php   "${wp_dir}/wp-content/mu-plugins/p006-wave1k-core-install-network-deny.php"
