...
function p006cInput(string $mode): array
{
    p006cAssert(
        ...
        'Usage: php p006-wave1c-wordpress-runtime-evidence.php <prepare|verify|aggregate>'
    );
    ...
    p006cAssert(preg_match('/^[0-9a-f]{40}$/', $sourceSha) === 1, 'Source SHA must be exact lowercase hex.');
    foreach (['Free SHA-256' => $freeHash, 'Pro SHA-256' => $proHash, 'pair ID' => $pairId] as $label => $hash) {
        p006cAssert(preg_match('/^[0-9a-f]{64}$/', $hash) === 1, "{$label} must be exact lowercase hex.");
    }
...
function p006cWriteJson(string $path, array $payload): void
{
    ...
    p006cAssert(file_put_contents($path, $json) !== false, "Unable to write evidence JSON: {$path}");
}
...
function p006cReadJson(string $path): array
{
    ...
    p006cAssert($content !== false, "Unable to read evidence JSON: {$path}");
    try {
    ...
    p006cAssert(is_array($decoded), "Expected JSON object at {$path}");
    return $decoded;
}
...
function p006cHashFile(string $path): string
{
    ...
    p006cAssert(is_string($hash), "Unable to hash artifact: {$path}");
    return $hash;
}
...
function p006cArtifactIdentity(array $input): array
{
    ...
    p006cAssert(is_file($installedRoot . '/wpessential.php'), 'Packaged Free plugin is not installed canonically.');
    p006cAssert(
        !file_exists((string) $input['wordpress_dir'] . '/wp-content/plugins/wpessential-pro'),
        'Pro package must remain absent from the Free-only runtime fixture.'
    );
    p006cAssert(class_exists(ZipArchive::class), 'ZipArchive is required for installed-byte verification.');
    ...
    foreach ($iterator as $file) {
        if ($file instanceof SplFileInfo && $file->isFile()) {
            $actualFiles[] = str_replace('\\', '/', substr($file->getPathname(), strlen($installedRoot) + 1));
        }
    }
...
PHP;
    p006cAssert(file_put_contents($path, $config . PHP_EOL) !== false, 'Unable to create disposable wp-config.php.');
}

/** @param array<string,mixed> $input */
function p006cEnsureNetworkProbe(array $input): void
{
    ...
    $plugin = <<<'PHP'
<?php
if (!defined('WPE_P006_NETWORK_DENY_PROBE_ACTIVE')) {
    define('WPE_P006_NETWORK_DENY_PROBE_ACTIVE', true);
}
add_filter(
    'pre_http_request',
...
PHP;
    p006cAssert(
        file_put_contents($dir . '/p006-wave1c-network-deny.php', $plugin . PHP_EOL) !== false,
        'Unable to install disposable runtime network-deny probe.'
    );
}
...
function p006cResetNetworkLog(array $input): void
{
    ...
    p006cAssert(file_put_contents($log, '') !== false, 'Unable to reset runtime network log.');
}
...
function p006cServerEnvelope(string $context): void
{
    $uris = [
    ...
    $_SERVER['SCRIPT_NAME'] = $context === 'admin' ? '/wp-admin/admin.php' : '/index.php';
...
function p006cEnvironment(array $input): array
{
    ...
    p006cAssert(isset($wp_version) && is_string($wp_version), 'WordPress version is unavailable after core boot.');
    ...
    p006cAssert($wp_version === $input['expected_wordpress'], "WordPress cell mismatch: {$wp_version}.");
    p006cAssert($php === $input['expected_php'], "PHP cell mismatch: {$php}.");
    p006cAssert(str_starts_with($mysql, (string) $input['expected_mysql']), "MySQL cell mismatch: {$mysql}.");
    return [
...
function p006cPrepare(array $input): array
{
    ...
    p006cEnsureNetworkProbe($input);
    p006cResetNetworkLog($input);
    p006cServerEnvelope('install');
    ...
    p006cAssert(defined('WPE_P006_NETWORK_DENY_PROBE_ACTIVE'), 'Runtime network-deny MU-plugin did not load.');
    ...
    p006cAssert(is_blog_installed(), 'Disposable WordPress installation did not reach installed state.');

    $plugin = 'wpessential/wpessential.php';
    if (is_plugin_active($plugin)) {
        deactivate_plugins($plugin, true, false);
    }
    p006cAssert(!is_plugin_active($plugin), 'Free plugin must begin clean activation from inactive state.');
    $activation = activate_plugin($plugin, '', false, true);
    p006cAssert(!is_wp_error($activation), 'WordPress activation API rejected packaged Free candidate.');
    p006cAssert(is_plugin_active($plugin), 'Packaged Free candidate is not active after activation.');

    $networkAttempts = p006cNetworkAttempts($input);
    p006cAssert($networkAttempts === [], 'Unexpected outbound WordPress HTTP during install/activation: ' . implode(', ', $networkAttempts));
    return [
        'status' => 'PASS',
        'phase' => 'activation',
...
function p006cVerify(array $input): array
{
    ...
    $environment = p006cEnvironment($input);
    $plugin = 'wpessential/wpessential.php';
    p006cAssert(is_plugin_active($plugin), 'Packaged Free candidate is not active in fresh request.');
    p006cAssert(class_exists(\WPEssential\Bootstrap\Plugin::class), 'Free Platform bootstrap class did not resolve.');

    $kernel = \WPEssential\Bootstrap\Plugin::kernel();
    p006cAssert($kernel instanceof \WPEssential\Kernel\Kernel, 'WPEssential kernel is unavailable.');
    p006cAssert($kernel->isBooted(), 'WPEssential kernel did not boot in real WordPress.');
    $modules = $kernel->modules();
    p006cAssert($modules->has('custom-post-types'), 'Free Custom Post Types module is missing.');
    p006cAssert($modules->has('taxonomies'), 'Free Taxonomies module is missing.');
    ...
    foreach ([
        ...
        p006cAssert(!defined($proConstant), "Free-only runtime manufactured Pro state: {$proConstant}");
    }
    p006cAssert(
    ...
    $networkAttempts = p006cNetworkAttempts($input);
    p006cAssert($networkAttempts === [], 'Unexpected outbound WordPress HTTP during runtime boot: ' . implode(', ', $networkAttempts));
    return [
        'status' => 'PASS',
...
function p006cAggregate(array $input): array
{
    ...
    return [
        ...
        'side_effect_profile' => [
            ...
            'runtime_network_attempts_detected' => $networkAttempts !== [],
            'runtime_network_attempts' => array_values(array_unique($networkAttempts)),
            'pro_runtime' => false,
