...
function gVerify(array $in): array {
    $activation=gRead($in['evidence_dir'].'/activation.json'); gAssert(($activation['status']??null)==='PASS','Activation evidence not PASS'); gNetworkProbe($in); gReset($in['network_log']); $c=$in['context']; gEnvelope($c); gContext($c); ob_start(); require $in['wp_dir'].'/wp-load.php'; $bootstrap=(string)ob_get_clean(); require_once ABSPATH.'wp-admin/includes/plugin.php'; $env=gEnvironment($in); $runtime=gRuntime($in); gNoLeak($bootstrap,$in); $f=$in['fixture_id'];
    ...
    elseif($f==='FP-28'){gAssert(($c==='cron'&&defined('DOING_CRON')&&DOING_CRON===true)||($c==='cli'&&defined('WP_CLI')&&WP_CLI===true),'Background context marker absent');$obs=['background_context'=>$c,'php_sapi'=>PHP_SAPI,'bootstrap_nonfatal'=>true,'premium_modules_registered'=>[],'premium_migrations_allowed'=>false,'premium_implementation_files_loaded'=>[]];}
