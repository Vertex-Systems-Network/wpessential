...
| GOV-OWNER-CONSENT-P001-TEMP-P006-004 | TEMPORARY_MATRIX | P-001/CF → P-006 Wave 1F | **ACTIVE** | One-tranche executable environment matrix exactly for P-006 Wave 1F FP-19/20 on WordPress `6.9` + PHP `8.2` + MySQL `8.4` and WordPress `7.1` + PHP `8.5` + MySQL `8.4`; disposable/sandbox exact-head mismatch evidence only. From the verified immutable Free artifact, two NON-RELEASE / TEST-ONLY Free variants may be derived by changing only the plugin `Version` header plus `WPE_VERSION`: `0.0.9` for FP-19 and `0.1.1` for FP-20. Each derived Free ZIP and derived pair identity must be independently SHA-256 pinned. | Does **not** authorize FP-21+; permanent P-001/CF certification; Free/Pro pair or P-006 runtime certification; ADR-0010 acceptance; live entitlement/provider/billing/payment/license/allocation calls; production credentials/data; irreversible/destructive operations; deploy/release; or changes to Platform API/schema, runtime implementation, compatibility evaluator, Pro artifact, normal release artifacts or release-builder rules merely to manufacture fixture PASS. Owner wording is durably recorded in Issue #991 comment `5680135691`; governance record work is Issue #992. |
...
Wave 1F temporary P-001/CF matrix + TEST-ONLY candidate-variant grant: **ACTIVE / `GOV-OWNER-CONSENT-P001-TEMP-P006-004` / FP-19/20 only**.
P-006 evidence truth before Wave 1F execution: **144 documented / 14 executed / 14 PASS / 0 FAIL / 0 certified Free/Pro pairs / 0 runtime certifications**.
...
The fourth temporary P-001/CF matrix grant is a distinct one-tranche authorization for Wave 1F FP-19/20 only on the same two disposable environment cells. Because repository history contains no real immutable out-of-range Free marketing-version artifacts, it additionally permits two derived NON-RELEASE / TEST-ONLY Free variants from the verified immutable Free artifact: `0.0.9` for FP-19 and `0.1.1` for FP-20. Only the Free plugin `Version` header and `WPE_VERSION` may change in those disposable derived candidates; Platform API/schema, runtime implementation, compatibility evaluator, Pro artifact bytes, release artifacts and release-builder rules remain unchanged. Each derived artifact/pair must be independently hashed and may not be promoted as a release artifact or certification claim. This authority does not extend to FP-21+ or any excluded live/production/destructive/deploy scope.
