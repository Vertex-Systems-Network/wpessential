...
| `P0-M00-WP06` | Webhooks & Connections signature/replay/Event Inbox/provider evidence protocol | `DONE` | `INTEGRATION` | `SERIALIZE` | Connection Registry + Safe HTTP + Webhook Gateway + Event Inbox | ADR-0122; WC-01…WC-156 documented; 0 executed; I4/I5=0; EI topology open. |
...
| `P0-M00-WP08` | P-005 Secrets Vault crypto/recovery executable evidence refinement | `SPECIFICATION` | `BLOCKING_FOUNDATION` | `SERIALIZE` | Vault + Policy + Audit + Backup/restore + Connections/credentials + Multisite | Current planning work; inspect ADR-0048/0085 + generic P-005 and create fixed protocol only if no equivalent exists. |
...
|---:|---|---|---|
| 1 | P-005 Secrets Vault crypto/recovery evidence refinement | `SPECIFICATION` current | Critical foundation; accepted architecture exists, generic P-005 exists, dedicated fixed adversarial protocol not yet found |
| 2 | Reassess P-002 UI, P-007 CI, P-008 build, P-012 Membership, P-013 Backup and other unresolved blockers | `BLOCKED` by sequence | Choose by critical-path value after Vault planning closes |
...
Current future-development critical foundations remain P-001 compatibility, P-003 Job, P-004 Definition, P-005 Vault, P-007 CI, P-008 build, P-009 Query, P-010 Relations, P-011 Workflow, P-012 Membership and P-013 Backup according to the Implementation Readiness Matrix.
...
Continue `P0-M00-WP08`: reconcile accepted Vault envelope/key-slot/use-grant architecture with the generic P-005 spike, Backup disaster-recovery semantics, Connections credential usage, Multisite ownership and privacy/logging, then create a fixed executable evidence protocol if no equivalent exists.
