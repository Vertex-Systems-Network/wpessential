...
This register contains unresolved implementation profiles/evidence only. Accepted architecture/product/security decisions are preserved in ADRs through **ADR-0059**.
...
Accepted logical resources and trust separation. Open: OpenAPI contract, RFC 9457 problem-type catalog, pagination/idempotency exact headers, rate-limit policy, compatibility negotiation, service test environment and field-level privacy/retention implementation.
...
Accepted C0–C4 model; **34 target destinations, 0 certified**. Need protocol adapters, provider version/profile registry, upload/resume/integrity/finalization/delete/retention/restore evidence.
...
ADRs **0035–0059** preserve accepted core semantics through JobService execution/fairness/backpressure. ADR-0006 remains open only for concrete backend/Action Scheduler evidence, not for re-opening the accepted JobService semantics without a superseding ADR.
...
4. **Do not install, compile, migrate, benchmark, test, generate crypto keys, contact provider APIs, create queue actions or run workers before explicit owner consent.**
...
5. P-003 exact future evidence protocol derived from ADR-0059 without executing it.
