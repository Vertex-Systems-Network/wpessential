...
final class QueryFieldRelationShortCircuitTest extends TestCase
{
    public function testRelationEmptyShortCircuitStripsFieldWithoutCallingFieldsOwnerOrProvider(): void
    {
        ...
        self::assertSame(
            ['policy', 'relation.describe', 'read:11', 'read:13', 'relation.matching'],
            $trace->getArrayCopy(),
        );
        self::assertNotContains('field.describe', $trace->getArrayCopy());
        self::assertNotContains('field.matching', $trace->getArrayCopy());
        self::assertNotContains('provider', $trace->getArrayCopy());
    }

    private function planner(ArrayObject $trace): QueryAuthorizedPlanner
    {
        ...
        $checker = new class($trace) implements CapabilityCheckerInterface {
        ...
        $relations = new class($trace, $this->relationId()) implements RelationQueryConsumerInterface {
    ...
    private function provider(ArrayObject $trace): QueryProviderExecutorInterface
    {
        return new class($trace) implements QueryProviderExecutorInterface {
            ...
            public function execute(QueryProviderPlan $plan): QueryExecutionResult
            {
                $this->trace[] = 'provider';
    ...
    private function definition(): QueryDefinition
    {
        return new QueryDefinition(
            identity: [
                'uuid' => '01990f6e-1f30-4000-8000-000000000205',
                'key' => 'posts.relation-field-short-circuit',
                'name' => 'Relation and Field short circuit',
                'revision' => 1,
                'lifecycle' => 'draft',
            ],
            astVersion: 1,
