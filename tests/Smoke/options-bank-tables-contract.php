...
foreach ($files as $file) {
    ...
    foreach ($localRecords as $record) {
        ...
        if (!is_string($id) || !str_starts_with($id, 'tables.') || isset($records[$id])) {
            throw new RuntimeException(sprintf('Surface 7 record IDs must be unique tables.* values; invalid %s.', is_scalar($id) ? (string) $id : 'value'));
        }
        if (!is_string($optionPath) || $optionPath === '' || isset($paths[$optionPath])) {
            throw new RuntimeException(sprintf('Surface 7 option_path values must be non-empty and unique; invalid %s.', is_scalar($optionPath) ? (string) $optionPath : 'value'));
        }
        $records[$id] = $record;
...
if (count($records) !== 165) {
    throw new RuntimeException(sprintf('Surface 7 Options Bank must contain exactly 165 records; found %d.', count($records)));
}
if ($rejected !== 14 || $deferred !== 5 || $exceed !== 10) {
    throw new RuntimeException(sprintf('Surface 7 policy counts drifted (rejected=%d deferred=%d exceed=%d).', $rejected, $deferred, $exceed));
}

$requiredSafetyIds = [
    'tables.security.generic_sql_console',
    'tables.security.arbitrary_ddl',
    'tables.security.drop_truncate_console',
    'tables.security.raw_identifier',
    'tables.security.privilege_bypass',
    'tables.security.cross_site_access',
    'tables.security.core_table_mutation',
    'tables.security.unvalidated_mutation',
    'tables.portability.unbounded_export',
    'tables.migration.raw_sql_transform',
    'tables.migration.destructive_without_recovery',
    'tables.ai.autonomous_destructive',
];
foreach ($requiredSafetyIds as $id) {
