...
if (!is_array($requiredFamilies) || count($requiredFamilies) !== 7) {
    throw new RuntimeException('Surface 7 market audit must define exactly seven canonical capability families.');
}
...
foreach ($providers as $provider) {
    ...
    if (!is_array($mapped) || !is_array($na)) {
        throw new RuntimeException(sprintf('Provider %s is missing family mapping metadata.', $provider['id']));
    }
    $covered = [];
    foreach ($mapped as $family => $recordIds) {
        if (!isset($requiredFamilySet[$family]) || !is_array($recordIds) || $recordIds === []) {
            throw new RuntimeException(sprintf('Provider %s has an invalid family mapping for %s.', $provider['id'], (string) $family));
        }
        ++$familyMappings;
        $covered[$family] = true;
        foreach ($recordIds as $recordId) {
            if (!is_string($recordId) || !isset($records[$recordId])) {
                throw new RuntimeException(sprintf('Provider %s references missing Bank record %s.', $provider['id'], is_scalar($recordId) ? (string) $recordId : 'value'));
            }
            ++$bankReferences;
        }
    }
...
foreach ($specialists as $provider) {
    foreach (($provider['bank_record_ids'] ?? []) as $recordId) {
        if (!is_string($recordId) || !isset($records[$recordId])) {
            throw new RuntimeException(sprintf('Specialist provider references missing Bank record %s.', is_scalar($recordId) ? (string) $recordId : 'value'));
        }
        ++$bankReferences;
    }
}
...
$expected = [
    ...
    'family_mappings' => 19,
    'non_applicable_family_cells' => 9,
    'bank_record_references' => 80,
    'extra_dispositions' => 6,
    'unresolved' => 0,
];
