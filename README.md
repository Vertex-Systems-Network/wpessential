...
Audit anchor for this README reconciliation: `main @ f253f9ed653e8aadf05a52420ce87436e44a9990` on **2026-09-07**. Repository/machine evidence remains authoritative if this prose later becomes stale.
...
The current conflict-safe queue is `config/coordination/agent-work-queue.json`. Its **2026-09-07** reconciliation records the CSV encoder (#235), remaining-gap audit (#236) and bounded internal export orchestration (#240) as completed. The single-row Fields inline-edit lane remains claimed in PR #237 and is not mergeable until its JavaScript lint/Architecture Guards failure is fixed. Fields bulk mutation remains dependency-blocked until #237 is exact-head certified and promoted. Workers must refresh current `main` before claiming anything.
