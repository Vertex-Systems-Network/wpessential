...
final class BuilderWidgetDefinition
{
    public const TYPE = 'builder-widget';
    public const OWNER_SURFACE_ID = 16;

    public static function assertOwned(Definition $definition): void
    {
        if ($definition->ownerSurfaceId !== self::OWNER_SURFACE_ID) {
            throw new InvalidArgumentException('Builder Widget definition owner surface is invalid.');
        }
        if ($definition->type !== self::TYPE) {
            throw new InvalidArgumentException('Builder Widget definition type is invalid.');
        }
    }
