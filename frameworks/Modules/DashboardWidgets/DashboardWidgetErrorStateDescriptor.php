...
final readonly class DashboardWidgetErrorStateDescriptor
{
    ...
    public function __construct(
        ...
        $expectedKeys = match ($this->blueprintId) {
            ...
            default => throw new InvalidArgumentException(
                'Dashboard Widget error-state Blueprint is outside the bounded V1 allowlist.',
            ),
        };

        if ($this->blueprintRevision !== DashboardWidgetComponentBlueprintCatalog::REVISION) {
            throw new InvalidArgumentException('Dashboard Widget error-state Blueprint revision must equal canonical revision 1.');
        }
        ...
        if ($actualKeys !== $expectedKeys) {
            throw new InvalidArgumentException(
                'Dashboard Widget error-state bindings must exactly match the trusted Blueprint schema.',
            );
        }
        ...
        foreach ($this->bindings as $key => $value) {
            if (!is_string($key) || !is_string($value)) {
                throw new InvalidArgumentException('Dashboard Widget error-state bindings must be string values.');
            }
            if (
                strlen($value) < 1
                ...
                throw new InvalidArgumentException(
                    'Dashboard Widget error-state binding strings must contain 1..2048 encoded bytes.',
                );
            }
            if (preg_match('/<\?(?:php|=)|<script\b|javascript:/i', $value) === 1) {
                throw new InvalidArgumentException(
                    'Executable authored channels are not accepted as Dashboard Widget error-state bindings.',
                );
            }
        ...
        if (!is_string($encoded) || strlen($encoded) > self::MAX_ENCODED_BYTES) {
            throw new InvalidArgumentException('Dashboard Widget error-state object exceeds the bounded 4096-byte limit.');
        }
    }
