...
final readonly class CustomPostTypeImportAbilityHandler implements AbilityHandlerInterface
{
    ...
    public function handle(array $input, ExecutionContext $context): mixed
    {
        ...
        if (!is_array($record) || array_is_list($record)) {
            throw new InvalidArgumentException('Imported CPT definition must be an object/map.');
        }

        $strategy = $input['strategy'] ?? 'create_only';
        if (!is_string($strategy) || !in_array($strategy, ['create_only', 'update_existing'], true)) {
            throw new InvalidArgumentException('CPT import strategy must be create_only or update_existing.');
        }
        ...
        if ($existing instanceof Definition) {
            ...
            if ($strategy !== 'update_existing') {
                throw new RuntimeException('CPT import found the same UUID with different target content.');
            }
    ...
    private function sourceDefinition(array $record): Definition
    {
        ...
        if (!is_string($id)
            ...
            throw new InvalidArgumentException('Imported CPT definition metadata is invalid or unsupported.');
        }

        $status = DefinitionStatus::tryFrom($statusValue);
        if (!$status instanceof DefinitionStatus) {
            throw new InvalidArgumentException('Imported CPT definition status is invalid.');
        }

        $normalizedDependencies = [];
        foreach ($dependencies as $dependency) {
            if (!is_string($dependency)) {
                throw new InvalidArgumentException('Imported CPT dependencies must contain UUID strings only.');
            }
        ...
        if (!hash_equals($source->computedChecksum(), $checksum)) {
            throw new InvalidArgumentException('Imported CPT definition checksum does not match its payload.');
        }
    ...
    {
        if ($definition->type !== CustomPostTypeDefinitionProjector::DEFINITION_TYPE
            || $definition->ownerSurfaceId !== CustomPostTypeDefinitionProjector::OWNER_SURFACE_ID
        ) {
            throw new RuntimeException('CPT import UUID is already owned by another canonical definition surface.');
        }
    }
    ...
    private function assertExpectedRevision(array $input, Definition $existing): void
    {
        ...
        if (!is_int($expected) || $expected !== $existing->revision) {
            throw new RuntimeException(sprintf(
                'CPT import write conflict: expected revision %d.',
                $existing->revision,
            ));
        }
    }
    ...
    private function assertKeyAvailable(Definition $source, ?Definition $existing): void
    {
        ...
        if (!is_string($key) || trim($key) === '') {
            throw new InvalidArgumentException('Imported CPT payload requires post_type_key.');
        }
        $key = trim($key);

        foreach ($this->definitions->byType(CustomPostTypeDefinitionProjector::DEFINITION_TYPE) as $candidate) {
            if ($candidate->ownerSurfaceId !== CustomPostTypeDefinitionProjector::OWNER_SURFACE_ID
            ...
            if (is_string($candidateKey) && trim($candidateKey) === $key) {
                throw new RuntimeException(sprintf(
                    'CPT import key collision: "%s" belongs to a different definition UUID.',
                    $key,
                ));
            }
        }
    ...
    private function assertValidationAllowsMutation(array $report): void
    {
        ...
        throw new InvalidArgumentException('CPT validation blocked the imported definition.');
    }
