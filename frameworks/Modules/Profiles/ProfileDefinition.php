...
final class ProfileDefinition
{
    ...
    public const OWNER_SURFACE_ID = 14;

    public static function assertOwned(Definition $definition): void
    {
        if ($definition->ownerSurfaceId !== self::OWNER_SURFACE_ID) {
            throw new InvalidArgumentException('Profile definition owner surface is invalid.');
        }
        if ($definition->type !== self::TYPE) {
            throw new InvalidArgumentException('Profile definition type is invalid.');
        }
    }
