...
function sourceByRef(
	...
	return bootstrap.sources.find( ( source ) => source.sourceRef === sourceRef ) ?? null;
}

function fieldByRef( source: QuerySource, fieldRef: string ): QueryField | null {
...
export function buildQueryAstPreview(
	...
	if ( source === null ) {
		return { ast: null, issues: [ 'Select a registered Query data source.' ] };
	}

	const projection = state.projection.filter(
		( ref, index, values ) =>
			fieldByRef( source, ref ) !== null && values.indexOf( ref ) === index
	);
	...
	if (
		...
		issues.push( `Page size must be between 1 and ${ source.maxPageSize }.` );
	}
	...
	if ( state.filterField !== '' || state.filterOperator !== '' || state.filterValue !== '' ) {
		...
		if ( field === null ) {
			issues.push( 'Choose a filter field declared by the selected data source.' );
		} else if ( ! operatorAllowed ) {
			issues.push( 'This filter operator is not advertised by the selected data source.' );
		} else if ( state.filterOperator === 'in' || state.filterOperator === 'not_in' ) {
			const values = state.filterValue
				.split( ',' )
				.map( ( value ) => normalizeScalar( value, field.logicalType ) );
			if ( values.length === 0 || values.some( ( value ) => value === null ) ) {
				issues.push( 'Set filters require a comma-separated list of valid values.' );
			} else {
				filter = {
					type: 'set_membership',
					field_ref: field.ref,
			...
			const value = normalizeScalar( state.filterValue, field.logicalType );
			if ( value === null ) {
	...
	if ( state.orderField !== '' ) {
		if ( fieldByRef( source, state.orderField ) === null ) {
			issues.push( 'Choose an ordering field declared by the selected data source.' );
		} else {
			orderBy.push( {
...
export function mountQueryScaffold(
	...
	const title = element( 'h2', 'wpessential-query__title', 'Query definition' );
	...
	const projectionLegend = element( 'legend', undefined, 'Projection fields' );
	...
	filterGroup.append( element( 'legend', undefined, 'Optional bounded filter' ) );
	...
	orderDirection.append( option( 'asc', 'Ascending' ), option( 'desc', 'Descending' ) );
	...
	const execute = element( 'button', 'button button-primary', 'Preview execution unavailable' );
	execute.type = 'button';
	...
	const renderSourceControls = (): void => {
		...
		for ( const field of source.fields ) {
			...
			wrapper.append( checkbox, document.createTextNode( ` ${ field.label }` ) );
		...
		for ( const predicate of SUPPORTED_FILTER_OPERATORS ) {
			if ( source.predicates.includes( predicate ) ) {
				filterOperator.append( option( predicate, predicate.replaceAll( '_', ' ' ) ) );
			}
		}
		...
		if ( source.supportsRelations ) {
			unavailable.push( 'Relation authoring is not exposed by this first admin scaffold.' );
		}
		if ( source.predicates.some( ( predicate ) => ! SUPPORTED_FILTER_OPERATORS.includes(
			predicate as ( typeof SUPPORTED_FILTER_OPERATORS )[ number ]
		) ) ) {
			unavailable.push( 'Some source predicates require a later dedicated control and remain intentionally unavailable.' );
		}
	...
	const renderPreview = (): void => {
		...
		status.textContent = 'Draft structure is locally well formed. Server-side Query validation is still required before save or execution.';
	...
	sourceSelect.addEventListener( 'change', () => {
		...
		const source = sourceByRef( bootstrap, state.sourceRef );
		state.projection = source?.fields[ 0 ] ? [ source.fields[ 0 ].ref ] : [];
		state.filterField = '';
...
function boot(): void {
	...
	try {
		const bootstrap = parseQueryBootstrap( JSON.parse( script.textContent ?? '{}' ) );
		if ( bootstrap === null ) {
			root.dataset.wpessentialEnhanced = 'invalid-bootstrap';
			root.textContent = 'Query editor is unavailable because its bootstrap contract is invalid.';
			return;
		}
